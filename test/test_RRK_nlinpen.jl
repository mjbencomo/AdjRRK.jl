...
@testset "derivative test" begin
    ...
    # errs_γ0,rate_γ0,h = Addj.derv_test!(RRK_solver!,arrks,arrks_h,ts,rk,h0,Nref)
    # @test abs(rate_γ0[end]-1)<AdjRRK.DRV_TOL

    # # assuming dt is cnst in linearization
    # arrks.γ_cnst = false
    # arrks.dt_cnst = true
    # arrks_h = AdjRRK_struct(arrks)
    # @pack! arrks_h = f,df,η,∇η,Hη
    # errs_dt0,rate_dt0,h = Addj.derv_test!(RRK_solver!,arrks,arrks_h,ts,rk,h0,Nref)
    # @test abs(rate_dt0[end]-1)<AdjRRK.DRV_TOL
    ...
    arrks_h = AdjRRK_struct(arrks)
    @pack! arrks_h = f,df,η,∇η,Hη
    errs,rate,h = AdjRRK.derv_test!(RRK_solver!,arrks,arrks_h,ts,rk,h0,Nref)
...
@testset "inner product test" begin
    # assuming γ is cnst in linearization
    arrks.γ_cnst = true
    ipt_γ0 = AdjRRK.ip_test!(RRK_solver!,arrks,ts,rk)
    @test ipt_γ0<AdjRRK.IPT_TOL

    # with proper linearization
    arrks.γ_cnst = false
    ipt = AdjRRK.ip_test!(RRK_solver!,arrks,ts,rk)
